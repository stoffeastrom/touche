(function() {
	...
	T.add = function(name, gesture) {
		...
		T.prototype[name] = function(bindingObj) {
			...
			var inst = T.cache.get(this.elem),
				g = new T.Gesture(inst.context, this.elem, name, bindingObj);

			T.utils.extend(g, gesture);
			inst.context.addGesture(g);
			g.init(this.elem);
			return this;
		};
	};
	...
	T.GestureHandler.prototype.setPoints = function(event, touchList) {
		...
		if(T.utils.touch) {
			...
			index = this.pointerIds.indexOf(pointerId);
			if(index < 0 ) {
				index = this.pointerIds.push(pointerId) -1;
			}
			len = this.pointerIds.length;
			this.data.points.length = len;
			this.data.points[index] = new T.Point(event.pageX, event.pageY);
		} else {
