(function() {
	...
	T.Cache.prototype.add = function(elem, context) {
		/*var index = (index = this.data.indexOf(elem) === -1) ?
				this.data.length + 1 :
				index,
			data = this.data[index] = this.data[index] || { context: context };
		return data;*/
	};
	...
	T.Cache.prototype.get = function(elem) {
		...
		return index !== -1 ?
			this.data[index] :
			T.cache.add(elem, new T.GestureController(elem));*/
		var cache = this.data.filter(function(obj) {
			return obj.elem === elem;
		});
		if(cache.length === 0) {
			cache = this.data[this.data.push({ elem: elem, context: new T.GestureController(elem) }) -1];
		} else {
			cache = cache[0];
		}
		...
		for(key in gestures) {
			if(gestures.hasOwnProperty(key) && T.utils.isObject(gestures[key])) {
				...
				if(T.utils.isFunction(gesture[action]) && !gesture.cancelled) {
					console.log("gesture", gesture.type, this)
