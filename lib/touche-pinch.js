(function() {
	...
	T.pinch = function(elem, gesture) {
		...
		var inst = T.cache.get(elem),
			...
			angleThreshold = gesture.options.angleThreshold,
			startDistance,
			currentDistance,
			startAngle,
			currentAngle,
			...
			update: function(event, data) {
				if(this.hasMoreTouches(event)) {
					...
					if(!this.pinch.start.point2) {
						...
						data.scale = scale;
						startAngle = T.utils.getDeltaAngle(this.pinch.start.point2, this.pinch.start.point1);
						currentAngle = T.utils.getDeltaAngle(this.pinch.start.point2, this.pinch.current.point1);
						rotation = currentAngle - startAngle;

						if(rotation > angleThreshold) {
							inst.context.cancelGestures('pinch');
							return;
						}
